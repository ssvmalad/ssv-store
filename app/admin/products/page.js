...
export default function ProductsPage() {
  ...
  const bulkDelete = async () => {
    ...
    if (error) {
      ...
      showToast(`Successfully deleted ${selectedIds.length} products`);
  ...
  const bulkCategorize = async () => {
    ...
    if (error) {
      showToast('Error categorizing products: ' + error.message, 'error');
    } else {
      showToast(`Successfully updated category for ${selectedIds.length} products`);
  ...
  const handleDelete = async () => {
    ...
    else { showToast('Product removed successfully'); setProducts(products.filter(p => p.id !== productToDelete.id)); }
  ...
  const handleSubmit = async () => {
    ...
    if (editingId) {
      ...
      else { showToast('Product updated!'); fetchProducts(); setIsProductModalOpen(false); }
    } else {
      const { error } = await supabase.from('products').insert([payload]);
      if (error) showToast('Insert failed: ' + error.message, 'error');
      else { showToast('Product added!'); fetchProducts(); setIsProductModalOpen(false); }
    }
  };
    ...
    <div className="p-6 lg:p-8 flex flex-col h-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="relative w-full md:w-96">
          ...
          <input type="text" placeholder="Search inventory by name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map(product => {
            ...
            <div key={product.id} className="relative group">
              ...
              <div onClick={() => openEditModal(product)} className={`bg-[#160F0F] rounded-2xl border overflow-hidden group cursor-pointer hover:border-[#3D2828] transition flex flex-col shadow-lg hover:shadow-xl hover:shadow-[#D4AF37]/5 ${isSelected ? 'border-[#D4AF37]/80 ring-1 ring-[#D4AF37]/30' : 'border-[#2C1E1E]'}`}>
                <div className="aspect-[4/3] bg-[#1C1212] relative overflow-hidden">
                  {product.media && product.media.length > 0 && product.media[0].type === 'image' ? (
                    <img src={product.media[0].url} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
                  ) : (product.image_url || product.images?.[0]) ? (
                    <img src={product.image_url || product.images[0]} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
                  ) : (
                  ...
                  <div className="absolute top-3 left-10 bg-black/60 backdrop-blur-md border border-white/10 px-2.5 py-1 rounded-md text-[10px] font-mono uppercase tracking-wider text-[#D4AF37]">{product.category}</div>
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-3 backdrop-blur-[2px]">
                    <button onClick={(e) => { e.stopPropagation(); openEditModal(product); }} className="p-2 bg-white/10 hover:bg-white/20 text-white rounded-lg transition"><Edit2 className="w-4 h-4" /></button>
                    <button onClick={(e) => { e.stopPropagation(); confirmDelete(product); }} className="p-2 bg-rose-500/20 hover:bg-rose-500/40 text-rose-300 rounded-lg transition"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
                <div className="p-5">
                  <h3 className="font-semibold text-white text-base leading-tight line-clamp-1" title={product.name}>{product.name}</h3>
                  <div className="flex items-end justify-between mt-4">
                    <div>
                      <p className="text-xs text-[#A09393] uppercase tracking-wider mb-0.5 font-medium">Standard</p>
                      <p className="font-mono text-lg text-white font-semibold">₹{product.price?.toLocaleString()}</p>
                    </div>
                    <button onClick={(e) => { e.stopPropagation(); toggleAvailability(product.id, product.is_available); }}
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-medium transition ${product.is_available ? 'bg-emerald-950/30 border-emerald-900/50 text-emerald-400' : 'bg-rose-950/30 border-rose-900/50 text-rose-400'}`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${product.is_available ? 'bg-emerald-400 shadow-[0_0_5px_#34d399]' : 'bg-rose-400'}`}></div>
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex justify-end" onClick={() => setIsProductModalOpen(false)}>
          <div className="bg-[#120B0B] w-full md:w-[640px] h-full border-l border-[#2C1E1E] shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#2C1E1E] bg-[#160F0F] shrink-0">
              <h2 className="text-lg font-semibold text-white">{editingId ? 'Edit Product' : 'New Product'}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="space-y-4">
                ...
                >
                  <input id="media-file-input" type="file" multiple accept="image/*,video/*,audio/*" onChange={(e) => handleMediaUpload(e.target.files)} className="hidden" disabled={isUploading} />
                  {isUploading ? (
            ...
            <div className="px-6 py-5 border-t border-[#2C1E1E] bg-[#160F0F] flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2 cursor-pointer" onClick={() => setFormData(prev => ({...prev, is_available: !prev.is_available}))}>
                ...
                <span className="text-xs font-medium text-[#C5B3B3]">Publish Active</span>
              </div>
